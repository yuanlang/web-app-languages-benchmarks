...
const MSG_QUEUE_LENGTH: usize = 1000; //length of message queue
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for _i in 0 .. n {
        let (mut tx_to_client, mut rx_by_server) = mpsc::channel::<Vec<u8>>(MSG_QUEUE_LENGTH);
        let (mut tx_to_server, mut rx_by_client) = mpsc::channel::<Vec<u8>>(MSG_QUEUE_LENGTH);
        let send_bytes_copy = send_bytes.clone();
        let mut tx_to_aggregator_copy = tx_to_aggregator.clone();
